...
#define ADD_UART_PORT(port,baud,stop,chk,irq,irq_src,tx_port,tx_pin,rx_port,rx_pin,af_func,   tx_buf,tx_buf_size,rx_buf,rx_buf_size) \
				{port,					\
				RCU_##port,				\
				baud,					\
				USART_WL_8BIT,			\
...
#define EN_UART0_REMAP	0

//Ôö¼Ó´®¿ÚÇë°´Ë³ÐòÔö¼Ó£¬·ñÔòÐèÒªÐÞ¸ÄUARTx_IRQHandlerº¯Êý£¬ÒÔ¼°¶ÔÓ¦drv²ã
uart_init_tab_t g_uart_init_tab[]=
{
	#if EN_UART0_REMAP
		ADD_UART_PORT(USART0,115200 ,DEF_STOP  ,DEF_CHK    ,UART0_IRQ,USART0_IRQn,GPIOB,GPIO_PIN_6 ,GPIOB,GPIO_PIN_7, GPIO_AF_0,  uart0_tx_buf,UART0_TX_BUF_SIZE,uart0_rx_buf,UART0_RX_BUF_SIZE),	//bms <-> front
	#else
		ADD_UART_PORT(USART0,115200 ,DEF_STOP  ,DEF_CHK    ,UART0_IRQ,USART0_IRQn,GPIOA,GPIO_PIN_9 ,GPIOA,GPIO_PIN_10, GPIO_AF_1,  uart0_tx_buf,UART0_TX_BUF_SIZE,uart0_rx_buf,UART0_RX_BUF_SIZE),	//bms <-> front
	#endif
	
	ADD_UART_PORT(USART1,115200 ,DEF_STOP  ,DEF_CHK    ,UART1_IRQ,USART1_IRQn,GPIOA,GPIO_PIN_2 ,GPIOA,GPIO_PIN_3, GPIO_AF_1,  uart1_tx_buf,UART1_TX_BUF_SIZE, uart1_rx_buf,UART1_RX_BUF_SIZE),	//Î´ÓÃ
...
static void bsp_uart_config (uint8_t uart_port,uint32_t baud, uint32_t data_bits,uint32_t stop_bit,uint32_t chk_bit,uint8_t mode)
{
   ...
   // nvic_priority_group_set(NVIC_PRIGROUP_PRE1_SUB3);
    ...
    gpio_af_set(g_uart_init_tab[uart_port].uart_tx_port, g_uart_init_tab[uart_port].af_func, g_uart_init_tab[uart_port].uart_tx_pin);
    /* connect port to USARTx_Rx */
    gpio_af_set(g_uart_init_tab[uart_port].uart_rx_port, g_uart_init_tab[uart_port].af_func, g_uart_init_tab[uart_port].uart_rx_pin);
    
	/* configure USART Tx as alternate function push-pull */
    gpio_mode_set(g_uart_init_tab[uart_port].uart_tx_port, GPIO_MODE_AF, GPIO_PUPD_PULLUP, g_uart_init_tab[uart_port].uart_tx_pin);
    gpio_output_options_set(g_uart_init_tab[uart_port].uart_tx_port, GPIO_OTYPE_PP, GPIO_OSPEED_10MHZ, g_uart_init_tab[uart_port].uart_tx_pin);

    /* configure USART Rx as alternate function push-pull */
    gpio_mode_set(g_uart_init_tab[uart_port].uart_rx_port, GPIO_MODE_AF, GPIO_PUPD_PULLUP, g_uart_init_tab[uart_port].uart_rx_pin);
    gpio_output_options_set(g_uart_init_tab[uart_port].uart_rx_port, GPIO_OTYPE_PP, GPIO_OSPEED_10MHZ, g_uart_init_tab[uart_port].uart_rx_pin);
