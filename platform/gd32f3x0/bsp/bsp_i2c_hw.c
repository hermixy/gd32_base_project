...
#define ADD_I2C_ITEM(i2c_num, port, pin_scl, pin_sda, af_func, i2c_speed, i2c_addr)	\
					{							\
						i2c_num,				\
						port,					\
						GPIO_PIN_##pin_scl,		\
...
static i2c_item_t g_i2c_cfg_tab[]=
{
	...
	ADD_I2C_ITEM(I2C0,	GPIOB,	6 ,	7 ,		GPIO_AF_1,		100000, I2C0_SLAVE_ADDRESS7),
...
static void clear_all_i2c_err(uint8_t ch)
{
	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_SMBALT);
	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_SMBTO);
	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_PECERR);
	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_OUERR);
	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_AERR);
	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_LOSTARB);
	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_BERR);
	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_ADDSEND);
//	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_TFF);
//	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_TFR);
//	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_RFF);
//	i2c_flag_clear(I2C_CFG.i2c_num,		I2C_FLAG_RFR);
}
...
//	gpio_init(I2C_CFG.port, GPIO_MODE_AF_OD, GPIO_OSPEED_50MHZ, I2C_CFG.pin_sda);
    ...
    gpio_af_set(I2C_CFG.port, I2C_CFG.af_func, I2C_CFG.pin_scl);
    /* connect PB7 to I2C0_SDA */
    gpio_af_set(I2C_CFG.port, I2C_CFG.af_func, I2C_CFG.pin_sda);
    /* configure GPIO pins of I2C0 */
    gpio_mode_set(I2C_CFG.port, GPIO_MODE_AF, GPIO_PUPD_PULLUP,I2C_CFG.pin_scl);
    gpio_output_options_set(I2C_CFG.port, GPIO_OTYPE_OD, GPIO_OSPEED_50MHZ,I2C_CFG.pin_scl);
    gpio_mode_set(I2C_CFG.port, GPIO_MODE_AF, GPIO_PUPD_PULLUP,I2C_CFG.pin_sda);
    gpio_output_options_set(I2C_CFG.port, GPIO_OTYPE_OD, GPIO_OSPEED_50MHZ,I2C_CFG.pin_sda);
...
//	}
    ...
    i2c_ack_config(I2C_CFG.i2c_num, I2C_ACK_DISABLE);	//关闭应答
	
	clear_all_i2c_err(ch);
}
...
#define CHK_TIMEOUT(flag)   time_out_cnt = 1000*3; 								\
...
#define BUSY_CHK_TIMEOUT(flag)   time_out_cnt = 1000*3; 							\
							while(i2c_flag_get(I2C_CFG.i2c_num,flag))			\
							{   												\
								time_out_cnt--;									\
								if(time_out_cnt==0)								\
...
//-----------------------解锁i2c
void Delay_I2C(volatile uint32_t i)
{
  while(i--);
}

void Resume_IIC(uint8_t ch, volatile uint32_t time_out)
{
	rcu_periph_clock_enable(I2C_CFG.port_clock);
	rcu_periph_clock_disable(I2C_CFG.i2c_clock);

   do{
		//gpio_init(I2C_CFG.port, GPIO_MODE_OUT_OD, GPIO_OSPEED_50MHZ, I2C_CFG.pin_scl);
		//gpio_init(I2C_CFG.port, GPIO_MODE_OUT_OD, GPIO_OSPEED_50MHZ, I2C_CFG.pin_sda);
//		gpio_init(I2C_CFG.port, GPIO_MODE_OUT_OD, GPIO_OSPEED_50MHZ, I2C_CFG.pin_scl);
//		gpio_init(I2C_CFG.port, GPIO_MODE_OUT_OD, GPIO_OSPEED_50MHZ, I2C_CFG.pin_sda);
	    gpio_mode_set(I2C_CFG.port, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, I2C_CFG.pin_scl);
		gpio_output_options_set(I2C_CFG.port, GPIO_OTYPE_OD, GPIO_OSPEED_50MHZ, I2C_CFG.pin_scl);
		gpio_mode_set(I2C_CFG.port, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, I2C_CFG.pin_sda);
		gpio_output_options_set(I2C_CFG.port, GPIO_OTYPE_OD, GPIO_OSPEED_50MHZ, I2C_CFG.pin_sda);
	   
	   
		gpio_bit_reset(I2C_CFG.port, 	I2C_CFG.pin_scl);
		Delay_I2C(100);
		gpio_bit_reset(I2C_CFG.port, 	I2C_CFG.pin_sda);
		Delay_I2C(100);
		gpio_bit_set(I2C_CFG.port, 		I2C_CFG.pin_scl);
		Delay_I2C(100);
		gpio_bit_set(I2C_CFG.port, 		I2C_CFG.pin_sda);
		Delay_I2C(100);	
		
		if(time_out-- == 0) {return;}	//{break;} //
		
	}while( (!gpio_input_bit_get(I2C_CFG.port, I2C_CFG.pin_sda)) && (!gpio_input_bit_get(I2C_CFG.port, I2C_CFG.pin_scl)) );
	
	I2C_hw_init(ch);
	DBG_E_LIMIT(500, "IIC[%d] again init------time_out:%d", ch, time_out);
}


//---The standard I2C method to clear the bus is to have the host send up to 9 clocks until the slave releases the bus.
static void release_i2c_bus(uint8_t ch)
{
	static uint8_t i = 0;
...
uint8_t bsp_I2C_read(uint8_t ch,uint8_t addr,uint8_t reg,uint8_t* pbuf, uint8_t len)
{
	...
	//---------------------------xyh
	uint32_t err_flag = 0;
	volatile uint32_t time_out_cnt = 0;
	uint8_t i = 0;
	(void)err_flag;
	...
	//i2c_software_reset_config(I2C_CFG.i2c_num, I2C_SRESET_SET);
