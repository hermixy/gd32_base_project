...
void TFT_LCD_Init(void)
{
		...
		rcu_periph_clock_enable(LCD_RCU_PORTA);
		rcu_periph_clock_enable(LCD_RCU_PORTB);
		
		gpio_init(LCD_PORTA, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, DC_PIN);
		gpio_bit_set(LCD_PORTA, DC_PIN);
		
		gpio_init(LCD_PORTB, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, CLK_PIN|DIN_PIN|RES_PIN|BLK_PIN);
		gpio_bit_set(LCD_PORTB, CLK_PIN|DIN_PIN|RES_PIN|BLK_PIN);
	#endif

	OLED_RST_Clr();
	//lcd_delay_ms(20);
	block_ms(20);	//不加，可能导致初始化不成功
	OLED_RST_Set();
	//lcd_delay_ms(20);
	block_ms(20);	//不加，可能导致初始化不成功
	OLED_BLK_Set();
...
void LCD_ShowChar3216(u16 x,u16 y,u8 num,u8 mode,u16 color, uint16_t back_color)
{
	...
	uint32_t index = 0;
	u16 x0=x;   
    if(x>LCD_W-16||y>LCD_H-32)return;	    //设置窗口		   
	num=num-' ';//得到偏移后的值
	LCD_Address_Set(x,y,x+16-1,y+32-1);      //设置光标位置 
	if(!mode) //非叠加方式
	{
		index = num;
		index = num*64;
		...
		{ 
			temp=asc2_3216[index+pos];		 //调用3216字体
			for(t=0;t<8;t++)
			{                 
				if(temp&0x01)LCD_WR_DATA(color);
					else LCD_WR_DATA(back_color);//BACK_COLOR
					temp>>=1; 
					x++;
			}
			x=x0;
	...
	}else//叠加方式
	{
		...
		for(pos=0;pos<64;pos+=2)
		{
			...
			for(t=0;t<8;t++)
		    {
		        if(temp&0x01) {LCD_DrawPoint(x+t,y+pos/2,color);}//画一个点     
		        temp>>=1;
		    }
			temp=asc2_3216[index+pos+1];		 //调用3216字体
			for(t=0;t<8;t++)
		    {
		        if(temp&0x01) {LCD_DrawPoint(x+t+8,y+pos/2,color);}//画一个点     
		        temp>>=1;
		    }
		}
	}
...
void LCD_show_ok_ng_6432(u16 x,u16 y,u8 num,u8 mode,u16 color, uint16_t back_color)
{
	...
	if(!mode) //非叠加方式
	{
		...
		{
			temp=ascii_6432_OK_NG_TAB[index+pos];		 //调用6432字体
			for(t=0;t<8;t++)
			{
				if(temp&0x01)LCD_WR_DATA(color);
					else LCD_WR_DATA(back_color);//BACK_COLOR
...
void LCD_show_ok_ng_12864(u16 x,u16 y,u8 num,u8 mode,u16 color, uint16_t back_color)
{
	...
	uint32_t index = 0;
	u16 x0=x;   
    if(x>LCD_W-64||y>LCD_H-128)return;	    //设置窗口		   
	//num=num-' ';//得到偏移后的值
	LCD_Address_Set(x,y,x+64-1,y+128-1);      //设置光标位置 
	if(!mode) //非叠加方式
	{
		index = num;
		index = num*8*128;
		for(pos=0;pos<8*128;pos++)
		{
			temp=ascii_12864_OK_NG_TAB[index+pos];		 //调用12864字体
			for(t=0;t<8;t++)
			{
				if(temp&0x01)LCD_WR_DATA(color);
					else LCD_WR_DATA(back_color);//BACK_COLOR
...
void LCD_show_ok_ng_19296(u16 x,u16 y,u8 num,u8 mode,u16 color, uint16_t back_color)
{
	...
	uint32_t index = 0;
	u16 x0=x;   
    if(x>LCD_W-96||y>LCD_H-192)return;	    //设置窗口		   
	//num=num-' ';//得到偏移后的值
	LCD_Address_Set(x,y,x+96-1,y+192-1);      //设置光标位置 
	if(!mode) //非叠加方式
	{
		index = num;
		index = num*12*192;
		for(pos=0;pos<12*192;pos++)
		{
			temp=ascii_19296_OK_NG_TAB[index+pos];		 //调用12864字体
			for(t=0;t<8;t++)
			{
				if(temp&0x01)LCD_WR_DATA(color);
					else LCD_WR_DATA(back_color);//BACK_COLOR
...
void LCD_ShowString_xyh(u16 x,u16 y,u8 size,u8 mode, const u8 *p,u16 front_color, uint16_t back_color)
{
    while(*p!='\0')
    {
		if(size == 16)
		{
			if(x>LCD_W-8){x=0;y+=8;}
			if(y>LCD_H-16){y=x=0;LCD_Clear(RED);}
			LCD_ShowChar1608(x,y,*p, mode, front_color, back_color);
			x+=8;
		}
        else if(size == 32)
		{
			if(x>LCD_W-16){x=0;y+=16;}
			if(y>LCD_H-32){y=x=0;LCD_Clear(RED);}
			LCD_ShowChar3216(x,y,*p, mode, front_color, back_color);
			x+=16;
		}
        p++;
    }  
